...
export default function Cart() {
  ...
  useEffect(() => {
    if (items.length === 0) { setEstimate(null); setMsg(''); return; }
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const subscribedIds: string[] = subRes.data.data.map((s: { productId: string }) => s.productId);
  ...
  return (
    <div>
        ...
        <div key={item.productId} className="card">
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="text-sm text-muted mt-1">₹{(item.estimatedMonthlyCost || item.basePrice * 30).toFixed(2)}<span className="price-unit">/mo</span></div>
            </div>
            <button className="btn btn-sm btn-danger" onClick={() => removeItem(item.productId)}>{tr.cart_remove}</button>
          </div>
        </div>
      ))}
      ...
      {estimate && (
        <div className="card">
          <div className="card-section compact" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', marginBottom: '0.75rem' }}>
            <div className="font-semibold text-sm" style={{ color: '#166534' }}>{tr.prorated_title}</div>
            <div className="text-xs mt-1" style={{ color: '#166534' }}>
              {tr.prorated_body.replace('{days}', String(estimate.items[0]?.billableDays || 0)).replace('{month}', new Date().toLocaleString(locale === 'hi' ? 'hi-IN' : 'en-IN', { month: 'long' }))}
            </div>
          </div>
          ...
          <div className="card-section">
            {estimate.items.map((ei) => (
              <div key={ei.productId} className="flex justify-between text-sm" style={{ padding: '0.25rem 0' }}>
                <span>{ei.productName} ({ei.billableDays} {tr.cart_days_left.replace('{s}', ei.billableDays > 1 ? 's' : '')})</span>
            ...
            <div className="flex justify-between font-bold" style={{ padding: '0.5rem 0', marginTop: '0.5rem', borderTop: '1px solid var(--border)', fontSize: '1rem' }}>
              <span>{tr.cart_due_now}</span>
              <span>₹{estimate.totalAmount.toFixed(2)}</span>
            </div>
          </div>

          <button className="btn btn-primary btn-block mt-3" onClick={handleCheckout} disabled={checkingOut}>
            {checkingOut ? tr.cart_processing : tr.cart_pay.replace('{amount}', estimate.totalAmount.toFixed(2))}
          </button>
